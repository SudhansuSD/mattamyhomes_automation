...
export async function validateRedirectCases(
    ...
    const casesToValidate = typeof options?.limit === 'number'
        ? redirectCases.slice(0, options.limit)
        : redirectCases;

    expect(casesToValidate.length, `${options?.label ?? 'Redirect'} cases should be available`)
        .toBeGreaterThan(0);
    ...
    for (const redirectCase of casesToValidate) {
        await test.step(redirectCase.title, async () => {
            ...
            expect(
                finalUrl.href,
                `${sourceUrl} should not loop to the same legacy URL`
            ).not.toBe(sourceUrl);
        });
    }
}
