...
export async function installVisibleLeadFormFinder(
  ...
  await driver.execute(
    ({ containerSelectors, formTextPattern, globalName }) => {
    ...
    },
    {
      ...
      formTextPattern:
        options.formTextPattern ||
        'submit|first name|last name|email|zip|postal|community updates|get information',
...
export async function submitVisibleLeadFormByIndex(
  ...
  if (submitted) {
    await waitForLeadFormPostSubmitState(driver, pauseMs);
  }
...
export async function fillInvalidEmailLeadFormByIndex(
  ...
  const filled = await driver.execute(
    ({ globalName, index, invalidData }) => {
      ...
      const checkConsent = () => {
        const checkbox = Array.from<HTMLInputElement>(
          form.querySelectorAll('input[type="checkbox"]'),
        ).find(
          (candidate) =>
            !/real estate agent/i.test(
              `${candidate.getAttribute('aria-label') || ''} ${candidate.closest('label')?.textContent || ''}`,
            ),
        );
      ...
      fill(
        'input[name*="zip" i], input[id*="zip" i], input[placeholder*="Zip" i], input[placeholder*="Postal" i]',
        invalidData.zip,
      );
...
export async function fillValidLeadFormByIndex(
  ...
  const submitted = await driver.execute(
    ({ fillOptions, globalName, index }) => {
      ...
      form.scrollIntoView({ block: 'center', inline: 'center' });
      selectOption(
        'select[name*="community" i], select[id*="community" i], select',
        fillOptions.communityPattern,
      );
      fill(
        'input[name*="first" i], input[id*="first" i], input[placeholder*="First" i]',
        fillOptions.firstName,
      );
      fill(
        'input[name*="last" i], input[id*="last" i], input[placeholder*="Last" i]',
        fillOptions.lastName,
      );
      fill(
        'input[type="email"], input[name*="email" i], input[id*="email" i]',
        `${fillOptions.emailPrefix}_${Date.now()}@${fillOptions.emailDomain}`,
      );
...
async function waitForLeadFormPostSubmitState(
  driver: MobileDriver,
  timeout: number,
): Promise<void> {
  await driver
    .waitUntil(
      async () => {
        const errorSnapshot = await getLeadFormErrorSnapshot(driver);
        const submissionSnapshot = await getLeadFormSubmissionSnapshot(driver);

        return (
          errorSnapshot.invalidFieldCount > 0 ||
          /required|invalid|error|please enter|field is required|valid domain/i.test(
            `${errorSnapshot.text} ${errorSnapshot.emailValidationMessage} ${errorSnapshot.emailAriaInvalid}`,
          ) ||
          submissionSnapshot.hasSuccess ||
          submissionSnapshot.hasSuccessDialog
        );
      },
      {
        interval: 250,
        timeout,
        timeoutMsg: 'Lead form did not show validation or submission state after submit',
      },
    )
    .catch(() => undefined);
}
  ...
  return driver.execute(() => {
    ...
    return {
      ...
      emailValidationMessage:
        email instanceof HTMLInputElement ? email.validationMessage || '' : '',
  ...
  return driver.execute(() => {
    ...
    const visibleDialog = Array.from(
      ...
      return (
        ...
        rect.width > 0 &&
        rect.height > 0 &&
        /thank you for your interest|successfully submitted|submitted|received/i.test(
          element.textContent || '',
        )
      );
    });
    ...
    return {
      hasSuccess:
        /thank you for your interest in mattamy homes|thank you for your interest|successfully submitted|submitted|received/i.test(
          text,
        ),
