...
export async function fetchJiraIssueRequirement(ticketId: string): Promise<JiraIssueRequirement> {
    ...
    const description = adfToPlainText(fields.description) ||
        htmlToPlainText(renderedFields.description) ||
        '';
...
function extractAcceptanceCriteria(fields: Record<string, any>, renderedFields: Record<string, any>): string[] {
...
function isTextAttachment(attachment: JiraAttachment): boolean {
    return /text\/plain|application\/json|text\/csv/i.test(attachment.mimeType ?? '') ||
