...
- Name: headerNavigation.spec.ts >> Header Navigation - USA >> @chrome-only @smoke @regression | USA | Homebuying mega-menu should expose expected links
...
Call log:
  - waiting for locator('header').getByRole('button', { name: /^Homebuying$/i }).first() to be visible
