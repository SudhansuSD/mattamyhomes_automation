...
function analyzeRequirement(
  ...
  return {
    ...
    acceptanceCriteria: cleanCriteria.length
      ? cleanCriteria
      : deriveDefaultAcceptanceCriteria(redirectScenarios),
...
function buildRedirectScenarios(
  ...
  const scenarios: RedirectScenario[] = URLs.map((sourceUrl) => {
...
function deriveEdgeCases(URLs: string[], scenarios: RedirectScenario[]): string[] {
  ...
  if (scenarios.length) {
    ...
    edgeCases.push(
      'Every source URL listed in the ticket should be covered, including those only in attachments or comments.',
    );
  }
...
function buildOpenQuestions(
  ...
  if (
    ...
    issue.acceptanceCriteria.some((item) => /\[object Object\]/.test(item))
  ) {
    questions.push(
  ...
  if (!hasRulesFile) {
    questions.push(
      `No redirect-rules file found at data/jira/${issue.ticket}.redirect-rules.json; add path/query rewrite rules to generate redirect scenarios for this ticket.`,
    );
  }

  if (scenarios.length === 0) {
    questions.push(
      'No source URLs matched the configured redirect rules in the Jira description, comments, or attachments.',
    );
  }
