...
The daily smoke run starts at 04:30 UTC (10:00 IST) in `.github/workflows/playwright.yml`.
Every run - scheduled, push, or manual - emails its own report once the report
has been generated and published, whether or not tests failed. The scheduled
run's email shows times in IST. GitHub Actions schedules can start late, so the
email arrives whenever that day's run finishes.
